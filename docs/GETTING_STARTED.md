...
> ```text
> ⚙️  [1. Start Server] ──► [2. Register Agent] ──► [3. Run Loop / Step in Directory] ──► [4. Prompt Intent Trade] ──► [5. Configure & Audit] ──► [6. File Dispute & Auto-Disclose]
> ```
...
3. **Execute Trade Cycles (Directory Tab):** Click **Run Loop** (to start continuous automated trading cycles) or **Step** (to execute a single manual cycle) on your registered agent card. Watch the agent daemon calculate strikes, make DeepBook options calls, and publish encrypted telemetry timeline entries in real-time.
