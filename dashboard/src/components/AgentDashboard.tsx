...
export const AgentDashboard: React.FC<AgentDashboardProps> = ({ onSelectAgent, activeSession }) => {
  ...
  const [daemonStatus, setDaemonStatus] = useState<'UNKNOWN' | 'RUNNING' | 'STOPPED' | 'ERROR'>('UNKNOWN');
  const [daemonBalances, setDaemonBalances] = useState<{ sui: string; dUSDC: string } | null>(null);
  const [devConsoleLogs, setDevConsoleLogs] = useState<string>('System initialized. Ready for cloud operator connections.\n');
  const [isActionLoading, setIsActionLoading] = useState(false);

  useEffect(() => {
    localStorage.setItem('aura_daemon_url', daemonUrl);
  }, [daemonUrl]);

  useEffect(() => {
    localStorage.setItem('aura_admin_key', adminKey);
  }, [adminKey]);

  const addLog = (text: string) => {
    setDevConsoleLogs(prev => prev + `[${new Date().toLocaleTimeString()}] ${text}\n`);
  };
    ...
    <div className="space-y-7 py-4">
      ...
      {loading ? (
        ...
        <>
          ...
          {/* Cloud Operator Developer Settings Console */}
          <div 
            className="rounded-2xl border overflow-hidden transition-all duration-300 mb-7"
            style={{ background: 'var(--color-surface)', borderColor: 'var(--color-border)' }}
          >
            <button
              onClick={() => setShowDevConsole(prev => !prev)}
              className="w-full px-6 py-4 flex items-center justify-between font-semibold text-[14px] cursor-pointer hover:bg-black/5 dark:hover:bg-white/5 transition-colors"
              style={{ color: 'var(--color-text-primary)' }}
            >
              <div className="flex items-center gap-2">
                <Settings className="h-4 w-4 text-[var(--color-brand)] animate-spin-slow" />
                <span>🛡️ Cloud Operator Developer Console (Railway Control Panel)</span>
              </div>
              <span className="text-xs text-[var(--color-text-muted)] font-medium">
                {showDevConsole ? 'Hide Console' : 'Show Console'}
              </span>
            </button>

            {showDevConsole && (
              <div className="p-6 border-t space-y-6" style={{ borderColor: 'var(--color-border)' }}>
                <div className="grid gap-6 md:grid-cols-3 items-start">
                  
                  {/* Column 1: Connection & Authentication settings */}
                  <div className="space-y-4 md:col-span-1">
                    <h4 className="text-[11px] font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">Daemon Connection</h4>
                    
                    <div className="space-y-3">
                      <div className="space-y-1">
                        <label className="text-[10px] font-semibold text-[var(--color-text-muted)]">Daemon Control URL</label>
                        <input
                          type="text"
                          value={daemonUrl}
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] font-semibold text-[var(--color-text-muted)]">Admin API Key</label>
                        <input
                          type="password"
                          value={adminKey}
                      ...
                      <button
                        ...
                        className="w-full py-1.5 rounded-lg text-xs font-semibold border cursor-pointer hover:bg-black/5 transition-colors"
                  ...
                  {/* Column 2: Status & Control Actions */}
                  <div className="space-y-4 md:col-span-1">
                    <h4 className="text-[11px] font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">Control Actions</h4>
                    
                    <div className="space-y-4">
                      {/* Active Status Badge */}
                      <div className="p-3 rounded-xl border flex items-center justify-between bg-[var(--color-surface-2)]" style={{ borderColor: 'var(--color-border)' }}>
                        <span className="text-xs text-[var(--color-text-secondary)]">Execution Status:</span>
                      ...
                      {daemonBalances && (
                        <div className="p-3 rounded-xl border space-y-1 text-xs bg-[var(--color-surface-2)]" style={{ borderColor: 'var(--color-border)' }}>
                          <div className="flex justify-between font-semibold" style={{ color: 'var(--color-text-secondary)' }}>
                            <span>Owner SUI:</span>
                            <span className="font-mono text-[var(--color-text-primary)]">{daemonBalances.sui} SUI</span>
                          </div>
                          <div className="flex justify-between font-semibold" style={{ color: 'var(--color-text-secondary)' }}>
                            <span>Owner dUSDC:</span>
                            <span className="font-mono text-[var(--color-text-primary)]">{daemonBalances.dUSDC} dUSDC</span>
                          </div>
                        </div>
                      )}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button
                          ...
                          disabled={isActionLoading || daemonStatus === 'RUNNING' || daemonStatus === 'ERROR'}
                          className="py-2 rounded-lg text-xs font-bold text-white bg-emerald-600 hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-1 cursor-pointer transition-opacity"
                        >
                          <Play className="h-3 w-3 fill-current" /> Start Loop
                        </button>
                        <button
                          onClick={() => triggerDaemonAction('stop')}
                          disabled={isActionLoading || daemonStatus === 'STOPPED' || daemonStatus === 'ERROR'}
                          className="py-2 rounded-lg text-xs font-bold text-white bg-amber-600 hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-1 cursor-pointer transition-opacity"
                        >
                      ...
                      <button
                        ...
                        className="w-full py-2 rounded-lg text-xs font-bold text-white bg-indigo-600 hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-1.5 cursor-pointer transition-opacity"
                      >
                        <RefreshCw className="h-3.5 w-3.5 animate-spin-slow" /> Reclaim All Policy Funds
                      </button>
                    </div>
                  </div>

                  {/* Column 3: Live console logs log window */}
                  <div className="space-y-4 md:col-span-1">
                    <h4 className="text-[11px] font-bold uppercase tracking-wider text-[var(--color-text-secondary)]">Request/Response Audit Trace</h4>
                    
                    <div 
                      className="p-3.5 rounded-xl border font-mono text-[10px] space-y-1 h-[215px] overflow-y-auto"
                      style={{ background: 'var(--color-bg)', borderColor: 'var(--color-border)', color: 'var(--color-text-secondary)' }}
                    >
                      {devConsoleLogs.split('\n').map((log, idx) => (
                        <div key={idx} className={log.includes('Error') ? 'text-red-500' : log.includes('Success') ? 'text-emerald-500' : ''}>
                          {log}
                        </div>
                      ))}
                    </div>
